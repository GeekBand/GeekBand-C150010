...
namespace geek
{
	...
	GeekResult GeekDbStorage::LoadAll(OUTPARAM GeekDbMetadata & metadata, OUTPARAM std::map<std::wstring, std::wstring>& entries) {
		if (m_pstg == NULL)
			return GEEK_ERROR_NULLPOINTER;
		...
		while (hr == S_OK) {
			...
			if (GEEKSUCCEEDED(result)) {
				...
				entries.insert(GeekKeyValue(wszKey, wszValue));
